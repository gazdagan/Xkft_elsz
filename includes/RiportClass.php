...
class RiportClass {
    ...
    public function Riport_1_telephelynetto($riport_date) {
        ...
        $html .= '<thead><tr  class="success"><th colspan="33">'.$date_start.' - '.$date_end.' Időszak nettó bevételi rendelő szerinti bontásban (Árbevétel-jutalék=Rendelőben marad)</th></tr></thead>';
        $html .= '<thead><tr><th>Rendelők</th>';
            ...
            $sql11 = "SELECT telephely_neve FROM telephelyek";
            $result1 = $this->dbconn->query($sql11);
                        
            if ($result1->num_rows > 0){
                
                while($row2 = $result1->fetch_assoc()) {
                    
                $telephely = $row2["telephely_neve"];
  
                $html .= '<tr><th>'.$telephely.'</th>';
                   
                for ($day = 1; $day < 32 ; $day++ ){
                        $color = $day % 7;
                        $date = $riport_date.'-'.$day;
                        // bevétel lekérdezés
                        $sql1 = "SELECT sum(bevetel_osszeg) as sum_bevetel, sum(jutalek_osszeg) as sum_jutalek FROM napi_elszamolas WHERE telephely = '$telephely' AND "
...
public function Riport_2_Kezelok_Netto($riport_date) {
    ...
    $html .= '<table class="table table-bordered table-condensed" id="riport">';
        $html .= '<thead><tr  class="success"><th colspan="33">'.$date_start.' - '.$date_end.' Időszak bevételi orvosok-kezelők szerinti bontásban</th></tr></thead>';
