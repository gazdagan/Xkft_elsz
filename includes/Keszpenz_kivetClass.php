   ...
   public function Visualize_Kpkivet_Form() {
        ...
        . '<div class="col-sm-7">'
            .  '<select class="form-control"   name="kivet_oka" id="kifizetes_oka" >
                    <option value="Egyéb kivét">Egyéb kivét</option>        
                    <option value="Jutalék kifizetés">Jutalék kifizetés</option>
                </select>'
        . '</div></div>';
...
//        . '</div></div>'; 
    ...
    public function User_Post_Kpkivet(){
        ...
        if (isset($_POST["kivet_oka"]) && isset($_POST["kivet_atvevo"])&& isset($_POST["kivet_osszeg"]) and isset($_POST["szamla_kep"])) {
            ...
            if (isset($_POST["szamla_sorszam"]) AND empty($_POST["szamla_sorszam"])) {
               
               $kivet_szszam = $kivet_megjegyzes;
            } else {
               $kivet_szszam = test_input($_POST["szamla_sorszam"]);
            }
            
            
            $kivet_telephely = $_SESSION['set_telephely'];
            // ellenőrzés csak létező kp bevételből lehet kifizetni
            // if ($kpvan >= $kivet_osszeg){
                
                $sql = "INSERT INTO kp_kivet (kivevo_neve, kivet_oka,kivet_osszeg,kivet_datum,kivet_telephely,kivet_megjegyzes,kivet_atvevo,kivet_torolve,kivet_szszam)
                               VALUES ('$kivevo_neve','$kivet_oka','$kivet_osszeg','$date','$kivet_telephely','$kivet_megjegyzes','$kivet_atvevo','0','$kivet_szszam')";
...
//            }
    ...
    public function Kp_kivet_kivet_hpbizonylatra($date,$kivet_telephely){
        ...
        if ($result->num_rows > 0) {     
            while ($row = $result->fetch_assoc()) {
                ...
                $jogcim = $row["kivet_oka"];
                $szoveg = $row["kivet_atvevo"];
                $megjegyzes = $row["kivet_megjegyzes"];
                $kivet_osszeg = $row["kivet_osszeg"] * -1;
                $szszam = $row["kivet_szszam"];
                        
                $id = 'date';
                
                    $html .= '<tr style="height:19px;">           
                        <td class="s10" colspan="2"><input  style=""  type="text" placeholder="" value="'.$jogcim.'"/></td>
                        <td class="s10" colspan="2"><input  style="width:100px;" type="text" value="'.$szszam.'" /></td>
                        <td class="s10" colspan="2"><input  style="width:100px;"  id="'.$id.'" type="text" onfocus="today_date(this.id)" value="'.$date.'"  ></td>
                        <td class="s10" colspan="3"><input  style="" type="text" placeholder="" value="'.$megjegyzes.' - '.$szoveg.'" /></td>
                        <td class="s10" colspan="2" style ="padding-top:4px;" onkeyup="bizonylat_kitolt()"><input style="width:152px;" type="number" placeholder="" value="'.$kivet_osszeg.'"/><label for="input">Ft</label></td>
                    </tr>';
