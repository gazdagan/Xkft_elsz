...
private function munakido_table($date,$user,$riport_type){
    ...
    $cegnev = $cegadat->select_CegNeve($this->telephely);
    $cegcim = $cegadat->select_CegCim($this->telephely);
    $rendelo = $cegadat->select_RendeloCim($this->telephely);
    ...
    $html .= '<table class="waffle" cellspacing="0" cellpadding="0" id="riport">
      ...
      <tbody>
         ...
         <tr style="height:18px;">
            <td class="s1" colspan="2">Munkáltató: '.$cegnev.'</td>
            <td class="s2" colspan="5">  '.$cegcim.'</td>
            <td class="s1" colspan="2">Munkavégzés helye:</td>
            <td class="s2" colspan="5">'.$rendelo.'</td>
...
public function Admin_post_munkaido_havikonyeloiexpotrt_query(){
   ...
   if(isset($_POST["munkaido_month"]) AND isset($_POST["munkaido_konyveles_export"]) AND $_POST["munkaido_konyveles_export"] == "konyveles_all_table" ){
    ...
    $html = '<div class="container" id="riport"><table class="table table-bordered" id="riport">
        <thead >
            <tr><th colspan="10" style="text-align:center;"><h2> '.$cegnev.' - '.$mounth.' havi munkaidő összesítés</h2></th></tr>
            <tr>
...
private function Munkaido_telephelyi_bontasban ($mounth){
    ...
    // táblázat szélessége
     $html .= '<table class="table table-bordered" >
        <thead >
            <tr><th colspan="8" style="text-align:center;"><h2>'.$cegnev.' - '.$mounth.' havi munkaidő összesítés telephelyi bontásban</h2></th></tr>
            <tr>
                
                <th>Munkavállaló néve</th>
                <th>Munkakör <br> Státusz</th>';
                
                $sql1 = "SELECT * FROM telephelyek";
                $result1 = $this->dbconn->query($sql1);

                    if ($result1->num_rows > 0) {
                        while ($row = $result1->fetch_assoc()) {
                            $html .= '<th>' . $row["telephely_neve"] . '</th>';
                        }
                    } else {
                        echo "0 results";
                    }
        ...
        // output data of each row
            while($row = $result->fetch_assoc()) {
                ...
                $html .= '<tr><td>'.$row["real_name"].'</td><td>'.$row["MK_info"].'</td>';
                    ...
                    $sql2 = "SELECT * FROM telephelyek";
                    $result2 = $this->dbconn->query($sql2);

                        if ($result2->num_rows > 0) {
                            while ($row = $result2->fetch_assoc()) {
                                
                                $rendelo = $row["telephely_neve"];
                                $html .= '<td>'. $this->Munkaido_telephelyi_bontasban_SelectCount($CardNo, $mounth, $rendelo).'</td>';
                            }
                        } else {
                            echo "0 results";
                        }
                    ...
                    $result3 = $this->dbconn->query($sql3);
                        ...
                        if ($result3->num_rows > 0) {
                            while ($row = $result3->fetch_assoc()) {
                                ...
                                $rendelo = $row["telephely_neve"];
                                $html .= '<td>'. $this->Munkaido_telephelyi_bontasban_SelectCount('', $mounth, $rendelo).'</td>';
                            }
                        } else {
                            echo "0 results";
                        }
...
public static function CardNoDecToHex($CardNoDec){
    
    $CardNoHex = 0;
    
    if (strlen($CardNoDec) == 10){
    // kobnvertálás Dec->hex    
    $hex = dechex($CardNoDec);
    // srting sorrend változtatás
    $hexarr1 = str_split($hex, 2);
    $CardNHex = $hexarr1[3].$hexarr1[2].$hexarr1[1].$hexarr1[0];
     
    }else{$CardNHex = "Error 10 Digit";}
