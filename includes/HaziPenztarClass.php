...
class HaziPenztarClass {
    ...
    public function __construct() {
        ...
        $this->telephelycim ='error 4';
        
        if (isset($_SESSION['set_telephely'])) {

            $this->telephely = $_SESSION['set_telephely'];
                        
            $telephely_datas = new telephely_db();
                  
            $this->cegnev1 = $telephely_datas->select_CegNeve($this->telephely); 
            $this->cegnev2 = $telephely_datas->select_CegCim($this->telephely); 
            $this->adoszam = $telephely_datas->select_Adoszam($this->telephely);
            $this->telephelycim = $telephely_datas->select_RendeloCim($this->telephely); 
        ...
        // Adatbázist + jogcímet hozzá kell admin!    
            $pbizdb = new telephely_db();
            
            $bev_dbname = $pbizdb->select_bev_pbiz_name($this->telephely);
            $kiad_dbname = $pbizdb->select_ki_pbiz_name($this->telephely);
            $jogcim = $this->telephely." napi kp bevétel";
           // Teszt  echo '----'.$bev_dbname.'-----'.$this->telephely;
            $this->bev_dbnames = array($this->telephely => $bev_dbname);
            $this->kiad_dbnames = array($this->telephely => $kiad_dbname);
            $this->beveteli_jogcim = array($this->telephely => $jogcim);
            }
    }
   ...
   <tbody id="HTMLtoPDF">
      ...
      <tr style="height:20px;">
         ...
         <td class="s2"></td>
         <td class="s3" colspan="3"> '.$this->cegnev1.'</td>
         
        <td class="s3" colspan="4"> '.$this->telephelycim.'</td>
   ...
   <table class="waffle" cellspacing="0" cellpadding="0" id ="penztar_bizonylat">
      ...
      <tbody>
         ...
         <tr style="height:19px;">
            ...
            <td class="s6" ></td>
            <td class="s6" colspan="4"> '.$this->telephelycim.'</td>
            <td class="s6" ></td>
            <td class="s7" >Kelt:</td>
   ...
   <table class="waffle" cellspacing="0" cellpadding="0" id ="penztar_bizonylat">
      ...
      <tbody>
       ...
       $html .= '<tr style="height:19px;">           
        ...
        $kpkivetek = new Keszpenz_kivetClass();
                
        $html .= $kpkivetek->Kp_kivet_kivet_hpbizonylatra($this->date,$this->telephely); 
       
       // HA VAN KP KIVÉT ITT KELL MINUSZOLNI
       $lines =8;
