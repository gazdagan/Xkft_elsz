     ...
     $conn = DbConnect();
        ...
        echo '<div class="panel panel-danger">';
            ...
            echo '<div class="panel-body">';
                ...
                // számlaszám részelt
                echo '<div class="form-group" style="padding-bottom: 1cm;">
                        <label class="col-sm-3 control-label" >Számla szám:</label>
                        <div class="col-sm-7">';
                        
                        echo'<div class="input-group">'
                            . '<span class="input-group-addon"><i class="fa fa-list-ol" aria-hidden="true"></i></span>';
                            echo'<input class = "form-control" name="szamlaszam" type="text" placeholder="ABCD-1234"> ';
...
// admin időszaki összesítés form eredmény post
    function admin_post_select_query(){
        ...
        //$this->where = "WHERE telephely LIKE '%$telephely%' AND (date BETWEEN '$startdate' AND '$enddate') AND torolt_szamla = '$torolve' AND  rogzito LIKE '%$rogzito%' AND  kezelo_orvos_id LIKE '%$terapeuta%' AND  bevetel_tipusa_id LIKE '%$fizmod%' AND szolgaltatas_id LIKE '%$szolgaltatas%' ";
        $this->where = "WHERE telephely LIKE '%$telephely%' AND (date BETWEEN '$startdate' AND '$enddate') AND torolt_szamla = '$torolve' "
                . "AND  rogzito LIKE '%$rogzito%' AND  kezelo_orvos_id LIKE '%$terapeuta%' AND  bevetel_tipusa_id LIKE '%$fizmod%' "
                . "AND szolgaltatas_id LIKE '%$szolgaltatas%' AND paciens_neve LIKE '%$paciens_neve%' AND szamlaszam LIKE '%$szamlaszam%'";
...
public function draw_double_chart(){
 ...
 public function select_orvos_idoszaki_jutaléklista(){
                    ...
                    <div class="panel-heading">Orvos vagy kezelő kimutatása egészségügyi elláűtásról.</div>
                        <div class="panel-body">';
                            ...
                            //terapeuta
                             echo '<div class="form-group" style="padding-bottom: 0cm;">
                                     ...
                                     echo'<div class="input-group">'
                                         ...
                                         echo '<option value="allin">ALL IN - összes orvos, kezelő</option>';
                                         echo '</select></div>';
                            ...
                            echo'<div class="form-group">'
                                ...
                                $telephelycheckbox = new telephely_db();
                                echo $telephelycheckbox->checkbox_telephely() .'</div></div>'; 
                                ...
                                .    '<button type = "button" class = "btn btn-info" onclick="StartPrtintJutalekPage()"><i class="fa fa-print" aria-hidden="true"></i> Nyomtatás</button>'
                                .     '<button type = "button" class = "btn btn-info" onclick="copyClipboard()"><i class="fa fa-clipboard" aria-hidden="true"></i> Vágólapra</button></div>'
                                . '</div></div>';
        ...
        $this->post_select_orvos_idoszakijutaléklista_alldoctors();    
        
    }
    
    
    public function post_select_orvos_idoszakijutaléklista_alldoctors(){
        
        echo '<div id="riport">';
        if ( isset($_POST["select_terapeuta"]) AND strcmp($_POST["select_terapeuta"],"allin") == FALSE){
            
            // összes telephelyi orvos lekérdezése egyszerre
            $sql = "SELECT * FROM kezelok_orvosok";
            ...
            $result = $this->dbconn->query($sql);
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        $_POST["select_terapeuta"] = $row["kezelo_neve"];
        ...
        }elseif (isset($_POST["select_terapeuta"]) AND strcmp($_POST["select_terapeuta"],"allin") == TRUE){
    ...
    public function  post_select_orvos_idoszakijutaléklista(){
        $sumjutalek  = 0;
        $conn = DbConnect();
        if (isset($_POST["select_terapeuta"]) AND isset($_POST["enddate"]) AND isset($_POST["startdate"]) AND isset($_POST["szamla_vevo"]))
        {
            $kezelo = $_POST["select_terapeuta"];
            $enddate = $_POST["enddate"];
            $startdate = $_POST["startdate"];
            $telephely = $_POST["szamla_vevo"];
                        
                    
            if (isset($_POST["jutaleklista_tipus"]) AND $_POST["jutaleklista_tipus"] == "all_jutalek-kppartner") {

            $sql = "SELECT * FROM `napi_elszamolas` WHERE telephely = '$telephely' AND kezelo_orvos_id  = '$kezelo' AND (date BETWEEN '$startdate' AND '$enddate') AND jutalek_osszeg <> '0' AND torolt_szamla = '0' AND bevetel_tipusa_id <> 'kp-partner'";
            }else{
            
            $sql = "SELECT * FROM `napi_elszamolas` WHERE telephely = '$telephely' AND kezelo_orvos_id  = '$kezelo' AND (date BETWEEN '$startdate' AND '$enddate') AND torolt_szamla = '0'";    
                            
            }
            
            $ceg_adat = new telephely_db();
            $ceg_nev = $ceg_adat->select_CegNeve($telephely); 
            $ceg_cim = $ceg_adat->select_CegCim($telephely);
            $ceg_adosz = $ceg_adat->select_Adoszam($telephely);
            
            $result = $conn->query($sql);
            
                    echo'<div class="container" >
                                <h2>'.$kezelo.' kimutatása egészségügyi ellátásról</h2>
                                <p><b>Dátum : </b>'.$startdate.' - '.$enddate.' - '.$telephely.'</p>
                                <p><b>Számlavevő: </b>'.$ceg_nev.' - '.$ceg_cim.' - '.$ceg_adosz.'<br></p>    
...
//                    echo'</tr>';
                    ...
                    $kezelo_adatok = new orvosok_kezelok();
                    $kezelocegNev = $kezelo_adatok->select_kezelo_Cegnev($kezelo,$telephely);
                    $kezelocegCim = $kezelo_adatok->select_kezelo_Cegcim($kezelo, $telephely);
                    $kezelocegAdosz = $kezelo_adatok->select_kezelo_CegAdosz($kezelo, $telephely);
        
                    echo '<br><br><br><div class="col-sm-4" style="border-top: dotted 1px; text-align: center;"> Időszaki jutalék elszámolás rendben: '.$kezelo. '  </div><div class="col-sm-8" style="text-align: center;"><b>Számlakiállító:</b> '.$kezelocegNev.' - '.$kezelocegCim.' - '.$kezelocegAdosz.'</div></div><br>';
        }            
        mysqli_close($conn);          
    }
