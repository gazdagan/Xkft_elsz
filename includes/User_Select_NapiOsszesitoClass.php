...
include_once ("./includes/PenzugyiEllenorzesClass.php");
...
// reci lekérdezi a napo jutaélkot
    ...
    public function post_select_orvos_naipjutaléklista() {
        ...
        if (isset($_POST["kezelo_jutalek"])and  isset($_POST["jutaleklista_datum"]) and  isset($_POST["szamla_vevo"]) ) {
            ...
            if (isset($_POST["kezelo_kp-partner"])) {// ha  az  orvos kezelő kp partner
                ...
                $where = "WHERE telephely = '$szamla_vevo' AND "
...
//               
            ...
            $ceg_nev = $ceg_adat->select_CegNeve($this->telephely); 
            $ceg_cim = $ceg_adat->select_CegCim($this->telephely);
            $ceg_adosz = $ceg_adat->select_Adoszam($this->telephely);

            echo'<div class="container">
                                <h2>' . $kezelo . ' kimutatása egészségügyi ellátásról</h2>
                                <p><b>Számla vevő: </b>'.$ceg_nev.' - '.$ceg_cim.' - '.$ceg_adosz.'<br><b>Rendelés napja:</b> '. $selectdate . '</p>
                            <table class="table table-striped">
                            <thead>
                                <tr>
                                  <th>Páciens</th>
            ...
            $kezelocegNev = $kezelo_adatok->select_kezelo_Cegnev($kezelo, $szamla_vevo);
            $kezelocegCim = $kezelo_adatok->select_kezelo_Cegcim($kezelo, $szamla_vevo);
            $kezelocegAdosz = $kezelo_adatok->select_kezelo_CegAdosz($kezelo, $szamla_vevo);
            

            echo '<br><br><div class="col-sm-5" style="border-top: dotted 1px; text-align: center;"> ' . $selectdate . ' napi jutalék elszámolás rendben: ' . $kezelo . ' </div><div class="col-sm-7" style="text-align: center;"><b>Számlakiállító:</b> '.$kezelocegNev.' - '.$kezelocegCim.' - '.$kezelocegAdosz.'</div></div>';
...
//        . "date = '$this->date' AND torolt_szamla = '0' ORDER BY kezelo_orvos_id, date_bevetel ASC ";
        ...
        // az aktuális napi bevételek lekérdezése
        $sql = "SELECT * FROM napi_elszamolas where telephely = '$this->telephely' AND "
        . "date = '$this->date' AND torolt_szamla = '0' ORDER BY szamlaszam ASC ";
 ...
 public function Napizaras_Form(){
    ...
    function admin_post_zarasdate() {
        ...
        if (isset($_POST["zaras_date"]) AND isset($_POST["zaras_telephely"])){
            ...
            $puchek= new PenzugyiEllenorzesClass;
            $html .= $puchek->NapiPuCheckView($date,$telephely);
