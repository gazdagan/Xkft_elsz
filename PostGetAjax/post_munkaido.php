...
if (isset($_POST["CardSerialNo"]) AND  isset($_POST["LogTelephely"]) AND isset($_POST["LogStatus"])  ){
    ...
    $mkido = new telephely_db ();
    $erkezes_hour = $mkido->select_mkido_kezdes_ora($LogTelephely);
    $erkezes_minute = $mkido->select_mkido_kezdes_perc($LogTelephely);

// munkaidó kezdés beállítása korábban nem lehet beljelentkezni
    if ($hour <= $erkezes_hour AND $minute < $erkezes_minute){$accestime = false;} else {$accestime = true;}   
    ...
    if ($LogStatus != "NULL" AND $LogStatus != "Haviosszesito" AND $accestime){
         ...
         // Decimális kártyaszám chek + konverzió  Salgótarjánban nem kell
        if (strlen($CardSerialNo) == 10){
            $CardSerialNo = MunkaidoClass::CardNoDecToHex($CardSerialNo);
           
        }    

    // káryta szám ellenörzés hexa szám 8 karakter decimális 10
        ...
        echo '<div class="alert alert-warning alert-dismissible fade in">
                 <strong> Káryta használatának ellenörzése - '.$hour.':'.$minute.'<br> Munkaidő kezdés '.$erkezes_hour.':'.$erkezes_minute.' után lehet! </strong>
              </div>';
    }
