...
//setcookie("valid_user", "0", time() + 14400);     // bejelentkezĂ©s Ă©rvĂ©nyes ideje 4 Ăłra 
...
$telephely->admin_post_delete_telephely();
$telephely->admin_post_insert_telephely();

$user_post = new napi_elszamolas;
$user_post->user_post_insert_db();
$user_post->user_post_delete_db_row();
$user_post->user_post_atadas();     //ĂˇtadĂˇs Ă©s napvĂ©gi zĂˇrĂˇs
...
//$admin_post_telephely = new telephely_db;
...
<html>
    <head>
        ...
        if (isset($_SESSION["set_telephely"]))
            ...
            $cegnev = $cegadat->select_CegNeve($telephely);
            echo  $telephely.' - '.$cegnev;
            } 
        else { echo "Login - ELSZ"; }
        ?> </title>
        <meta charset="utf-8">
...
//            if (isset($_SESSION['valid_user']) && isset($_COOKIE['valid_user'])) {
              if (isset($_COOKIE['valid_user']) && isset($_COOKIE['type_user']) && isset($_COOKIE['rendelo_user']) && isset($_SESSION['valid_user'])){
                     echo 'ok';
                        $_SESSION['type_user'] = $_COOKIE['type_user'];
                        $_SESSION['real_name'] = $_COOKIE['valid_user'];
                        $_SESSION['set_telephely'] = $_COOKIE['rendelo_user'];
                ...
                if (isset($_SESSION['type_user'])) {
                    ...
                    //User - jutalĂ©Ă©kelszĂˇmolĂł orvos felhasznĂˇlĂł
                ...
                $login = new Authenticate($action, "post");
                $login->Validate();
